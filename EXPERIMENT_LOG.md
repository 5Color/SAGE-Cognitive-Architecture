...
- adapt gain은 거의 유지되었지만, 전체 성능 개선으로 이어지지는 않았다.
- phase_2에서는 성능이 개선되었지만 phase_0, phase_1, phase_3에서 성능이 떨어졌다.

### Problem

- recent_acc, recent_loss, phase_shift_score, uncertainty는 현재 규칙을 명확히 알려주지 못했다.
- context가 action 판단에 직접 섞이면서 noise로 작동했을 가능성이 있다.
- context는 observation에 단순 결합하기보다 router나 organ selection을 조절하는 gate로 사용해야 한다.
